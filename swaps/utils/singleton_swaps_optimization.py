...
__C.MATCH_FEATURES_KWARGS.floor_extraction.enabled = False  # if True, estimate a per-image background floor and subtract it from each quantified row's intensity_sum IN PLACE (both real targets and decoys) -- every existing downstream consumer (FDR.INT_THRES, DirectLFQ, decoy construction, min_peak_sum_intensity, ...) reads intensity_sum by name, so this repoints all of them at the corrected value with no other code changes needed. The pre-correction value is preserved as intensity_sum_without_floor_correction; floor_avg_noise_per_pixel, floor_n_removed_px, floor_background_estimate are also added for diagnostics. Computed independently per row's own raw image (each run, the consensus row, and every decoy), not shared across runs -- see image_processing.estimate_floor_background. Benchmarked (2026-09) on 100 low-intensity and 100 full-range HYE ECOLI/YEAST candidates: shifts median log2fc toward the true ratio for both organisms in both benchmarks and improves ~75-77/100 candidates' distance to target, but widens the ratio IQR (precision/accuracy tradeoff) -- not a clean win, evaluate before enabling for a real run.
__C.MATCH_FEATURES_KWARGS.floor_extraction.threshold = 2.0  # intensity cutoff (in the row's own raw units, post gaussian-smoothing) used to build the connectivity mask for skimage.remove_small_objects. Must be > 0: MATCH_FEATURES_KWARGS.denoise.clean's own threshold=0 convention is a no-op here, since a Gaussian's tails leave the whole image nonzero, so remove_small_objects would never have anything to remove regardless of min_size.
__C.MATCH_FEATURES_KWARGS.floor_extraction.min_size = 10  # skimage.remove_small_objects min_size (8-connectivity): connected components smaller than this, in the thresholded+smoothed image, are treated as noise and their average pre-removal intensity becomes the background-floor estimate for that image.
__C.MATCH_FEATURES_KWARGS.floor_extraction.gaussian_kwargs = ConfigurationNode()
__C.MATCH_FEATURES_KWARGS.floor_extraction.gaussian_kwargs.sigma = [1, 2]  # same default as MATCH_FEATURES_KWARGS.denoise.smooth.gaussian_kwargs.sigma; kept as its own independent setting (not auto-synced) since floor_extraction runs per-image rather than on the shared consensus and may warrant its own tuning.
__C.MATCH_FEATURES_KWARGS.floor_extraction.gaussian_kwargs.mode = "nearest"
