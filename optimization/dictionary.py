...
def LoadMZML(msconvert_file:str):
	""" 
...
def ExtractPeak(x:np.ndarray, 
    ...
    if return_summary:
        return peak_result, peak_result.shape[0], peak_result.peak_width.mean(), peak_result.peak_width.std()
    else:
        return peak_result

# TODO: main method is moved to Dict class, consider archieve it
def AlignMZ(anchor:pd.DataFrame, 
    ...
    match method:
        case '2stepNN':
            ...
            primaryAlignment = pd.merge_asof(left=anchor.sort_values(col_to_align[0]), 
                                             right=primaryIsotope.sort_values(col_to_align[1]), 
                                             left_on=col_to_align[0], right_on=col_to_align[1], 
                                             tolerance=mz_tol, 
                                             direction='nearest').dropna(axis=0) # type: ignore
            if primaryAlignment.shape[0] > 0: 
                ...
                secondaryAlignment = pd.merge_asof(left=anchor.sort_values(col_to_align[0]), 
    ...
    if verbose:
        ...
        return n_matchedIso, AbundanceNotObs, IsKept, mzDelta_mean, mzDelta_std, None, None


def _get_RT_edge(precursorRow:pd.Series,
    ...
    MS1Intensity = pd.DataFrame({'mzarray_obs':MS1Scans.iloc[ScanIdx, :]['mzarray'], 
                                'intensity':MS1Scans.iloc[ScanIdx, :]['intarray']})
    if IsInLastScan is None:
        _, _, IsInLastScan, _, _, _, _ = AlignMZ(anchor=MS1Intensity, 
                                                 precursorRow=precursorRow, 
                                                 verbose=False, 
                                                 method='peakRange',
                                                 AbundanceMissingThres=AbundanceMissingThres)
    if IsInLastScan:
        Logger.debug('Is in scan %s and search for %s scan %s', ScanIdx, direction, ScanIdx+direction)
        ScanIdx += direction
        MS1Intensity = pd.DataFrame({'mzarray_obs':MS1Scans.iloc[ScanIdx, :]['mzarray'], 
                                 'intensity':MS1Scans.iloc[ScanIdx, :]['intarray']})
        _, _, IsInThisScan, _, _, _, _ = AlignMZ(anchor=MS1Intensity, 
                                                 precursorRow=precursorRow, 
                                                 verbose=False, 
                                                 method='peakRange',
                                                 AbundanceMissingThres=AbundanceMissingThres)
        ...
        if ScanIdx <= ScanIdx_right and ScanIdx >= ScanIdx_left:
            Logger.debug('Is not in scan %s and search for %s scan %s', ScanIdx, -direction, ScanIdx-direction)
            ScanIdx -= direction
            MS1Intensity = pd.DataFrame({'mzarray_obs':MS1Scans.iloc[ScanIdx, :]['mzarray'], 
                                 'intensity':MS1Scans.iloc[ScanIdx, :]['intarray']})
            _, _, IsInThisScan, _, _, _, _ = AlignMZ(anchor=MS1Intensity, 
    ...
    match (int(IsInLastScan)+int(IsInThisScan)):
        ...
        case 0|2: # consecutive N or Y
            return _get_RT_edge(precursorRow=precursorRow, 
                                MS1Scans=MS1Scans, 
...
def _search_BiDir_scans(precursorRow:pd.Series,
                        MS1Scans:pd.DataFrame,
                        ScanIdx:int,
                        ScanIdx_left:int,
                        ScanIdx_right:int,
    ...
    :AbundanceMissingThres:
    """
    if ScanIdx-step >= ScanIdx_left: # ensure search limit, only use left because of symmetricality

        MS1Intensity_next_left = pd.DataFrame({'mzarray_obs':MS1Scans.iloc[ScanIdx-step, :]['mzarray'], 
                                'intensity':MS1Scans.iloc[ScanIdx-step, :]['intarray']})
        _, _, IsInNextLeft, _, _, _, _ = AlignMZ(anchor=MS1Intensity_next_left, 
                                        precursorRow=precursorRow, 
                                        verbose=False, 
                                        method='peakRange',
                                        AbundanceMissingThres=AbundanceMissingThres)
        MS1Intensity_next_right = pd.DataFrame({'mzarray_obs':MS1Scans.iloc[ScanIdx+step, :]['mzarray'], 
                    'intensity':MS1Scans.iloc[ScanIdx+step, :]['intarray']})
        _, _, IsInNextRight, _, _, _, _ = AlignMZ(anchor=MS1Intensity_next_right, 
                                        precursorRow=precursorRow, 
                                        verbose=False, 
                                        method='peakRange',
                                        AbundanceMissingThres=AbundanceMissingThres)
        ...
        match (IsInNextLeft, IsInNextRight):
            case (0, 0):
                Logger.debug('Precursor %s not observed in scan %s and %s, search with increased step', \
                              precursorRow['id'], ScanIdx-step, ScanIdx+step)
                step += 1
                return _search_BiDir_scans(precursorRow,
                                        MS1Scans,
                                        ScanIdx,
                                        ScanIdx_left=ScanIdx_left,
                                        ScanIdx_right=ScanIdx_right,
                                        ...
                                        AbundanceMissingThres = AbundanceMissingThres)
            case (0, 1):
                Logger.debug('Precursor %s not observed in scan %s but in %s, search for right edge', \
                              precursorRow['id'], ScanIdx-step, ScanIdx+step)
                Left = ScanIdx + step
                Right = _get_RT_edge(precursorRow=precursorRow,
                                    MS1Scans=MS1Scans,
                ...
                return Left, Right
            case (1, 0):
                Logger.debug('Precursor %s observed in scan %s but not in %s, search for left edge', \
                              precursorRow['id'], ScanIdx-step, ScanIdx+step)
                Right = ScanIdx - step
                Left = _get_RT_edge(precursorRow=precursorRow,
                                    MS1Scans=MS1Scans,
                                    ScanIdx=ScanIdx - step,
                                    direction=-1,
                                    ScanIdx_left=ScanIdx_left, 
                ...
                return Left, Right
            case (1, 1):
                Logger.warning('Precursor %s observed in equal distance scan %s and %s, \
...
def locate_RT_range(precursorRow:pd.Series,
                    MS1Scans:pd.DataFrame,
                    ScanIdx:int,
    ...
    Logger.debug('Start scan = %s, Scan edge = (%s, %s)', ScanIdx, ScanIdx_left, ScanIdx_right)
    MS1Intensity = pd.DataFrame({'mzarray_obs':MS1Scans.iloc[ScanIdx, :]['mzarray'], 
                                'intensity':MS1Scans.iloc[ScanIdx, :]['intarray']})
    ...
    _, _, IsInThisScan, _, _, _, _ = AlignMZ(anchor=MS1Intensity, 
                                            ...
                                            AbundanceMissingThres=AbundanceMissingThres)
    if IsInThisScan:
        Logger.debug('Precursor %s observed in scan %s, search for left and right edge',\
                    precursorRow['id'], ScanIdx)
        Left = _get_RT_edge(precursorRow, 
                            MS1Scans,
                            ScanIdx = ScanIdx-1, 
                            direction = -1,
                            ScanIdx_right=ScanIdx_right,
        ...
        Right = _get_RT_edge(precursorRow, 
                            MS1Scans,
                            ScanIdx = ScanIdx+1, 
                            direction = 1,
                            ScanIdx_right=ScanIdx_right,
        ...
        Logger.debug('Precursor %s is not observed in seeding Scan %s, start searching scan %s and %s.', \
                     precursorRow['id'],ScanIdx, ScanIdx-step, ScanIdx+step)
        return _search_BiDir_scans(precursorRow=precursorRow,
                            MS1Scans=MS1Scans,
                            ScanIdx=ScanIdx,
                            ScanIdx_left=ScanIdx_left,
                            ScanIdx_right=ScanIdx_right,
...
class Dict:
    def __init__(self,
        ...
        self.MS1Intensity = pd.DataFrame({'mzarray_obs':OneScan['mzarray'], 
                                'intensity':OneScan['intarray']})
        self.peak_results = ExtractPeak(np.array(self.MS1Intensity['mzarray_obs']), 
                            np.array(self.MS1Intensity['intensity']), 
                            rel_height = rel_height)
        CandidateByRT_iso = CandidateByRT[['id', 'IsoMZ', "IsoAbundance"]].\
            explode(['IsoMZ','IsoAbundance']).\
                astype({'IsoMZ':'float64','IsoAbundance':'float64'})
        
        CandidateByRT_iso = pd.merge_asof(CandidateByRT_iso.sort_values('IsoMZ'), 
                                          self.peak_results, 
                                          left_on='IsoMZ', 
                                          right_on='apex_mz', 
                                          allow_exact_matches=True,
                                          direction='nearest')
        CandidateByRT_iso['matched'] = (CandidateByRT_iso['IsoMZ'] >= CandidateByRT_iso['start_mz']) & \
            (CandidateByRT_iso['IsoMZ'] <= CandidateByRT_iso['end_mz'])
        self.IsoAbundanceByID = CandidateByRT_iso.groupby(['id', 'matched'])['IsoAbundance'].sum().reset_index()
        CandidateByRT_isoMatched = CandidateByRT_iso[(CandidateByRT_iso['matched'])]
        Logger.info('Number of candidates after isotope match %s', len(CandidateByRT_isoMatched['id'].unique()))
        if len(CandidateByRT_isoMatched['id'].unique()) > 0:
            CandidateByRT_isoFiltered = self.IsoAbundanceByID[self.IsoAbundanceByID['matched'] & 
                                                            (self.IsoAbundanceByID['IsoAbundance'] >= 
                                                            (1-self.AbundanceMissingThres)
                                                            )
                                                            ]
            self.filteredCandidateIdx = CandidateByRT_isoFiltered['id'].unique().tolist()
            Logger.info('Number of candidates after isotope abundance filter %s', 
                        len(self.filteredCandidateIdx))
            if len(self.filteredCandidateIdx) > 0:
                CandidateByRT_isoFiltered_sumAbundance = CandidateByRT_isoMatched\
                    [CandidateByRT_isoMatched['id'].isin(self.filteredCandidateIdx)].groupby(['id', 'apex_mz'])\
                    ['IsoAbundance'].sum().reset_index()
                dict = CandidateByRT_isoFiltered_sumAbundance.pivot(index='apex_mz', 
                                                                    columns='id', 
                                                                    values='IsoAbundance')
                self.dict = pd.merge(left = self.peak_results[['apex_mz']],
                                    right = dict,
                                    left_on='apex_mz',
                                    right_index=True,
                                    how='left').set_index('apex_mz')
                self.dict.loc[0] = 1-self.dict.sum(axis=0) # append mz for accumulated not observed iso abundance
                self.dict = self.dict.fillna(0)
                Logger.debug('[Double check] Number of candidates by RT and abundance filter %s', 
                            len(self.dict.columns.values))
                
                self.obs_int = pd.DataFrame({'mzarray_obs': np.append(self.peak_results['apex_mz'].values, 
                                                                    [0], # append 0 mz for not observed iso abundance
                                                                    axis = 0), 
                                            'intensity': np.append(self.peak_results['peak_intensity_sum'].values, 
                                                                    [0], 
                                                                    axis=0)})
            else:
                self.dict = None
    ...
    def plot_observe_iso_abundance(self):
        self.IsoAbundanceByID[self.IsoAbundanceByID['matched']]['IsoAbundance'].hist(bins=100)
        plt.title('Distribution of Observed Isotope Abundance')
        ymin,ymax = plt.ylim()
        plt.vlines(x = 1-self.AbundanceMissingThres, ymin=ymin, ymax=ymax, label = 'Abundance Thres',color = 'r')
    ...
    def get_feature_corr(self, 
        ...
        if plot_hmap:
            ...
            plt.show()
        self.sol = (corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool)).stack())
        self.high_corr_sol = self.sol[self.sol >= corr_thres] # type: ignore
        if calc_jaccard_sim:
            jaccard_sim = []
            for pairs, row in self.high_corr_sol.items():
                jaccard_sim.append(jaccard_score(self.dict.loc[:, pairs[0]]>0, 
                                                 self.dict.loc[:, pairs[1]]>0, 
                                                 average = 'binary'))
            self.high_corr_sol = pd.DataFrame({'Pearson R': self.high_corr_sol, 
        ...
        if plot_hist:
            ...
            plt.vlines(x = corr_thres, ymin=ymin, ymax=ymax, linestyles='dashed', color = 'r') 
            
        Logger.info('Number of candidate pairs with correlation larger than %s: %s', 
                    corr_thres, self.high_corr_sol.shape[0])
        tmp = self.high_corr_sol.reset_index()
        self.HighCorrCand = np.union1d(tmp['level_0'].unique(), tmp['level_1'].unique())
        Logger.info('Number of candidated involved in high correlation: %s', len(self.HighCorrCand))
        
# TODO: peakRange method is moved to Dict class, consider archieve  
def ConstructDict(CandidatePrecursorsByRT: pd.DataFrame, 
                  OneScan: Union[pd.DataFrame, pd.Series], 
    ...
    """
    MS1Intensity = pd.DataFrame({'mzarray_obs':OneScan['mzarray'], 
                                 'intensity':OneScan['intarray']})
    peak_results = None
    logging.debug('Prepare data.')

    if method == 'peakRange':
        peak_results = ExtractPeak(np.array(MS1Intensity['mzarray_obs']), 
                                   np.array(MS1Intensity['intensity']), 
                                   rel_height = rel_height)
        merge_key = 'apex_mz'
        CandidateDict = peak_results[[merge_key]]
        y_true = pd.DataFrame({'mzarray_obs': peak_results['apex_mz'], 
    ...
    AlignmentResult.loc[:, 'n_matchedIso'],  AlignmentResult.loc[:, 'AbundanceNotObs'], \
        AlignmentResult.loc[:, 'IsKept'], AlignmentResult.loc[:, 'mzDelta_mean'], \
            AlignmentResult.loc[:, 'mzDelta_std'], alignment, IsotopeNotObs = \
        zip(*CandidatePrecursorsByRT.apply(lambda row:AlignMZ(MS1Intensity, 
                                                              row, 
                                                              method=method, 
    ...
    for idx, precursor_idx in zip(filteredIdx, filteredPrecursorIdx):
        ...
        CandidateDict = pd.merge(CandidateDict, right, on=merge_key, how='outer').\
...
def compare_isotope_pattern(MQ_withIso:pd.DataFrame,
                            seq_A_idx:int,
                            seq_B_idx:int
                            ):
    
    plot_comparison(y_true=MQ_withIso.loc[MQ_withIso['id']==seq_A_idx, 'IsoAbundance'].values[0],
