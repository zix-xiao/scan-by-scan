...
if [ "$1" = "maxquant" ]; then
    ...
    python -u /cmnfs/proj/ORIGINS/protMSD/maxquant/protNMF/maxquant_handler.py -ml $2 -MQ $3 -p $p -rtgw $rtgw -n_noise $n_noise -mm $mm -dev "$dev" -isospec $isospec -initWithCos $initWithCos -IDwithCos $IDwithCos
elif [ "$1" = "mascot" ]; then
    echo " ==================ProtMSD pipeline with $1================== "
    python -u /cmnfs/proj/ORIGINS/protMSD/maxquant/protNMF/mascot_handler.py -ml $2 -MQ $3 -p $p -rtgw $rtgw -n_noise $n_noise -mm $mm -dev $dev
fi
deactivate
